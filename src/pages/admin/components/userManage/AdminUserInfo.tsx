		...
		<AdminLayout>
			...
			<Card>
				...
				<CardContent>
					{/* <article>
						<div>포인트 지급</div>
						<Button onClick={(e:any)=>{e.stopPropagation();handleOpenDialog()}}>지급</Button>
					</article> */}
					<article>
						<InfoWrapper>
							<div>id</div>
						...
						{/* <InfoWrapper>
							<div>성별</div>
							<div>{userById.gender === 'male' ? '남' : '여'}</div>
						</InfoWrapper>
						<Divider />
						<InfoWrapper>
							<div>생년월일</div>
							<div>{userById.birth}</div>
						</InfoWrapper>
						<Divider /> */}
						<InfoWrapper>
							<div>계정 생성일</div>
						...
						{/* <InfoWrapper>
							<div>깃허브 닉네임</div>
							<div>{userManageData.githubNickname ?? '깃허브 인증이 안된 유저'}</div>
						</InfoWrapper> */}
